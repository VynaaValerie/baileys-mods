...
function updateVersionJson(version: [number, number, number]): boolean {
	const filePath = join(ROOT_DIR, 'src/Defaults/wa-version.json')
	const content = {
		version
	}
	...
	try {
		...
		if (currentVersion[0] === version[0] && currentVersion[1] === version[1] && currentVersion[2] === version[2]) {
			console.log(`✓ wa-version.json already up to date`)
			return false
		}

		writeFileSync(filePath, JSON.stringify(content) + '\n')
		console.log(`✓ Updated wa-version.json: [${currentVersion.join(', ')}] → [${version.join(', ')}]`)
		return true
	} catch (error) {
		console.error(`✗ Failed to update wa-version.json:`, error)
		throw error
	}
}
...
function updateGenerics(version: [number, number, number]): boolean {
	...
	try {
		...
		const versionRegex = /const libVersion = \[(\d+),\s*(\d+),\s*(\d+)\]/
		const match = content.match(versionRegex)

		if (!match) {
			throw new Error('Could not find libVersion declaration in generics.ts')
		}
		...
		const newContent = content.replace(
			versionRegex,
			`const libVersion = [${version[0]}, ${version[1]}, ${version[2]}]`
		)
...
async function main() {
	...
	const updates = [
		updateVersionJson(result.version),
