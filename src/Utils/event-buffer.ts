 ...
 */
type VynaaEventData = Partial<VynaaEventMap>

const BUFFERABLE_EVENT_SET = new Set<VynaaEvent>(BUFFERABLE_EVENT)

type VynaaBufferableEventEmitter = VynaaEventEmitter & {
	/** Use to process events in a batch */
	process(handler: (events: VynaaEventData) => void | Promise<void>): () => void
	/**
 ...
 */
export const makeEventBuffer = (logger: ILogger): VynaaBufferableEventEmitter => {
	...
	// emit the raw event map as individual VynaaValerie events
	ev.on('event', (map: VynaaEventData) => {
		for (const event in map) {
			ev.emit(event, map[event as keyof VynaaEventMap])
		}
	})
	...
	return {
		process(handler) {
			const listener = async (map: VynaaEventData) => {
				await handler(map)
			}
		...
		emit<T extends VynaaEvent>(event: VynaaEvent, evData: VynaaEventMap[T]) {
			...
			if (event === 'messages.upsert') {
				const { type } = evData as VynaaEventMap['messages.upsert']
...
function append<E extends BufferableEvent>(
	...
	switch (event) {
		...
		case 'contacts.update':
			const contactUpdates = eventData as VynaaEventMap['contacts.update']
		...
		case 'messages.upsert':
			const { messages, type } = eventData as VynaaEventMap['messages.upsert']
		...
		case 'messages.update':
			const msgUpdates = eventData as VynaaEventMap['messages.update']
		...
		case 'messages.delete':
			const deleteData = eventData as VynaaEventMap['messages.delete']
		...
		case 'messages.reaction':
			const reactions = eventData as VynaaEventMap['messages.reaction']
		...
		case 'message-receipt.update':
			const receipts = eventData as VynaaEventMap['message-receipt.update']
			...
			break
		case 'groups.update':
			const groupUpdates = eventData as VynaaEventMap['groups.update']
