...
const libVersion = [2, 3000, 1035194821]
import type {
	VynaaEventEmitter,
	VynaaEventMap,
	ConnectionState,
...
export function bindWaitForEvent<T extends keyof VynaaEventMap>(ev: VynaaEventEmitter, event: T) {
	return async (check: (u: VynaaEventMap[T]) => Promise<boolean | undefined>, timeoutMs?: number) => {
		let listener: (item: VynaaEventMap[T]) => void
...
export const bindWaitForConnectionUpdate = (ev: VynaaEventEmitter) => bindWaitForEvent(ev, 'connection.update')

/**
 * Fetches the latest WhatsApp Web version from the upstream source.
 * Use to ensure your WA connection is always on the latest version
 */
export const fetchLatestVersion = async (options: RequestInit = {}) => {
	const URL = 'https://raw.githubusercontent.com/WhiskeySockets/Baileys/master/src/Defaults/index.ts' /* upstream WA version source */
	try {
		...
		if (versionMatch) {
			...
			throw new Error('Could not parse WA version from upstream source')
		}
	} catch (error) {
		return {
			version: libVersion as WAVersion,
			isLatest: false,
			error
		}
	}
...
/**
 * Stream errors generally provide a reason, map that to a VynaaValerie DisconnectReason
...
/** @deprecated Use `fetchLatestVersion` instead */
export const fetchLatestBaileysVersion = fetchLatestVersion
