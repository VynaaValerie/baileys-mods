...
describe('E2E Tests', () => {
	let sock: ReturnType<typeof makeVynaaSocket>
			...
			sock.ev.on('connection.update', update => {
				...
				if (connection === 'open') {
					...
					sock
						...
						.then(groups => {
							const group = Object.values(groups).find(g => g.subject === 'VynaaValerie Group Test')
							if (group) {
								groupJid = group.id
					...
					if (reason === DisconnectReason.loggedOut) {
						console.error('Logged out, please delete the vynaa_auth_info_e2e folder and re-run the test')
					}
	...
	test('should download a quoted videos message within a group', async () => {
		if (!groupJid) {
			console.warn('⚠️ Skipping group test because "VynaaValerie Group Test" was not found.')
			return
		}
